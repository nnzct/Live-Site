  ...
  return (
    <div className="space-y-12">
      <div className="flex justify-between items-end border-b border-primary/30 pb-4">
        <div>
          <h2 className="orbitron text-4xl font-black text-light tracking-tighter uppercase">Discovered Worlds</h2>
          <p className="text-primary-light uppercase tracking-widest text-xs mt-2 font-bold">발견된 행성 목록 - 탐사 가능 구역</p>
        </div>
        <div className="text-right">
          <span className="text-5xl orbitron font-bold text-secondary opacity-50">{stages.length}</span>
          <p className="text-[10px] uppercase text-light-dim font-bold">기록된 행성 수</p>
        </div>
      </div>
        ...
        {stages.map((stage) => (
          ...
          >
            <div className="h-48 overflow-hidden relative">
              ...
              />
              {!stage.isPublished && (
                <div className="absolute top-2 right-2 px-2 py-1 bg-yellow-600 text-[10px] font-bold text-white rounded">준비 중 (DRAFT)</div>
              )}
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-[10px] text-light-dim uppercase tracking-widest font-bold">
                <div className="bg-dark p-2 rounded truncate">중력: {stage.metadata.gravity}</div>
                <div className="bg-dark p-2 rounded truncate">대기 O2: {stage.metadata.atmosphere.o2}%</div>
              </div>
